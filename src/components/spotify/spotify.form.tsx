...
import {isLink, isSpotifyAlbumLink, isSpotifyLink, isSpotifyPlaylistLink,} from "../../utils/regex.util";
        ...
        >
            ...
            <button className="btn btn-wide " disabled={fromContext.loading}>
                ...
                <div hidden={progressValue === 0} dir={"auto"}>
                    <ProgressDownload valueProgress={progressValue} className={""} hideText={false}/>
                </div>
            </button>
        </form>
    );
...
async function downloadTrack(trackId: string, setProgressValue: any, button: Element) {
    return new Promise(async (resolve, reject) => {
        try {
            await spotifyService.download(trackId, (value: number) => {
                button.classList.remove("loading")
                if (value == 100) {
                    resolve(true)
                } else {
